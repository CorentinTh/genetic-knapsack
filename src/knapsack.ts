...
function generateItems({ itemCount, weightRange, valueRange }: { itemCount: number; weightRange: ValueRange; valueRange: ValueRange }) {
  const items: Item[] = _.times(itemCount, () => ({
...
function solveKnapsack({
  ...
  _.times(generationCount, (generation: number) => {
    ...
    const { individual: bestIndividual, fitness: bestFitness, weight } = getBestIndividual({ population, items, backpackCapacity });

    stats.push({ generation, bestFitness, bestIndividual, weight });
  });
